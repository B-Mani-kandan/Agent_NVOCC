...
export class CommonMasterComponent implements OnInit {
  ...
  HandleRowAction(event: { action: string; data: any }) {
    if (event.action === 'select') {
      this.ModifyNonAccountId = event.data.ConsigneeID;
  ...
  fillGeneralForm(row: any) {
    ...
    this.isGridVisible = false;
    this.ModifyNonAccountId = row.ConsigneeID || null;
  ...
  OnMasterSave(action: any): void {
    ...
    const data = {
      ...
      ConsigneeID: action === 'Update' ? this.ModifyNonAccountId : '',
    };
    ...
    this.agentService.NVOCC_Save_ConsigneeMaster(combinedPayload).subscribe({
      next: (res) => {
        if (res.Status === 'Success') {
          ...
          this.ModifyNonAccountId = res.ReturnConsigneeID;
          this.isModifyVisible = true;
          this.ClearForms();
        } else {
          this.messageService.add({
