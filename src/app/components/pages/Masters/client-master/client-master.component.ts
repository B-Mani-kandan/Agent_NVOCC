...
import { GENERAL_FIELDS } from './client-master-input-fields.config';
...
export class ClientMasterComponent implements OnInit {
  ...
  HandleRowAction(event: { action: string; data: any }) {
    if (event.action === 'select') {
      this.ModifyAccountId = event.data.AccountID;
      this.fillGeneralForm(event.data);
    }
  }
  fetchGridData(tab: string) {
    const columnMap: any = {
      GENERAL: ['select', 'AccountName', 'RegNo', 'CompanyType', 'Address1'],
    };
    ...
    this.agentService.Nvocc_ClientMaster(tab, payload).subscribe(
      (res: any) => {
  ...
  fillGeneralForm(row: any) {
    ...
    this.isGridVisible = false;
    this.ModifyAccountId = row.AccountID || null;
  ...
  extractGeneralRowKey(fieldId: string): string {
    return fieldId.replace(/^client_Gen_/, '');
  }
  ...
  OnMasterSave(action: any): void {
    ...
    const data = {
      ...
      AccountID: action === 'MODIFY' ? this.ModifyAccountId : '',
      SubTableID: action === 'MODIFY' ? this.SubTableID : '',
    };
    ...
    this.agentService.NVOCC_Save_ClientMaster(combinedPayload).subscribe({
      next: (res) => {
        if (res.Status === 'Success') {
          this.messageService.add({
            severity: 'success',
          ...
          this.ModifyAccountId = res.ReturnAccountID;
          this.isModifyVisible = true;
          this.ClearForms();
        } else {
          this.messageService.add({
