...
import { GENERAL_FIELDS } from './shipper-master-input-fields.config';
import { ToastModule } from 'primeng/toast';
import { MessageService } from 'primeng/api';
@Component({
  selector: 'app-shipper-master',
  standalone: true,
  imports: [
    CommonModule,
  ...
  templateUrl: './shipper-master.component.html',
  styleUrl: './shipper-master.component.css',
})
export class ShipperMasterComponent implements OnInit {
  CompanyId: string | undefined;
  FinanceYear: any | undefined;
  BranchID: any | undefined;
  ModifyPartyId: any | undefined;
  SubTableID: any | undefined;
  ...
  getMandatoryFields(templateId: string): (fieldId: string) => boolean {
    const mandatoryFields: Record<string, string[]> = {
      GENERAL: [
        'shipper_Gen_PartyName',
        'shipper_Gen_Address1',
        'shipper_Gen_Address2',
      ],
    };
  ...
  HandleRowAction(event: { action: string; data: any }) {
    if (event.action === 'select') {
      this.ModifyPartyId = event.data.PartyID;
  ...
  fetchGridData(tab: string) {
    const columnMap: any = {
      GENERAL: ['select', 'PartyName', 'EmailID', 'Address1', 'Address2'],
    };
    ...
    this.agentService.Nvocc_ShipperMaster(tab, payload).subscribe(
      (res: any) => {
  ...
  fillGeneralForm(row: any) {
    ...
    this.isGridVisible = false;
    this.ModifyPartyId = row.PartyID || null;
    this.SubTableID = row.SubTableID || null;
  ...
  extractGeneralRowKey(fieldId: string): string {
    return fieldId.replace(/^shipper_Gen_/, '');
  }
  ...
  OnMasterSave(action: any): void {
    ...
    const data = {
      ...
      PartyID: action === 'Update' ? this.ModifyPartyId : '',
      SubTableID: action === 'Update' ? this.SubTableID : '',
    };
    ...
    this.agentService.NVOCC_Save_ShipperMaster(combinedPayload).subscribe({
      next: (res) => {
        if (res.Status === 'Success') {
          this.messageService.add({
            severity: 'success',
          ...
          this.ModifyPartyId = res.ReturnPartyID;
          this.isModifyVisible = true;
          this.ClearForms();
        } else {
          this.messageService.add({
