...
import { GENERAL_FIELDS } from './consignee-master-input-fields.config';
...
@Component({
  ...
  styleUrl: './consignee-master.component.css',
})
export class ConsigneeMasterComponent implements OnInit {
  CompanyId: string | undefined;
  ModifyConsigneeId: any | undefined;
  ...
  getMandatoryFields(templateId: string): (fieldId: string) => boolean {
    const mandatoryFields: Record<string, string[]> = {
      GENERAL: [
        'consignee_Gen_ConsigneeName',
        'consignee_Gen_Address1',
        'consignee_Gen_Address2',
      ],
    };
  ...
  HandleRowAction(event: { action: string; data: any }) {
    if (event.action === 'select') {
      this.ModifyConsigneeId = event.data.ConsigneeID;
  ...
  fetchGridData(tab: string) {
    const columnMap: any = {
      GENERAL: ['select', 'ConsigneeName', 'EmailID', 'Address1', 'Address2'],
    };
    ...
    this.agentService.Nvocc_ConsigneeMaster(tab, payload).subscribe(
      (res: any) => {
  ...
  fillGeneralForm(row: any) {
    ...
    this.isGridVisible = false;
    this.ModifyConsigneeId = row.ConsigneeID || null;
  ...
  extractGeneralRowKey(fieldId: string): string {
    return fieldId.replace(/^consignee_Gen_/, '');
  }
  ...
  OnMasterSave(action: any): void {
    ...
    const data = {
      ...
      ConsigneeID: action === 'Update' ? this.ModifyConsigneeId : '',
    };
    ...
    this.agentService.NVOCC_Save_ConsigneeMaster(combinedPayload).subscribe({
      next: (res) => {
        if (res.Status === 'Success') {
          this.messageService.add({
            severity: 'success',
          ...
          this.ModifyConsigneeId = res.ReturnConsigneeID;
          this.isModifyVisible = true;
          this.ClearForms();
        } else {
          this.messageService.add({
